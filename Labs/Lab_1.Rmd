...
6.1 se utilizaron las primeras 4 columnas sumatoria de nota y 3 de nota como se consulto
```{r}
students_projection <- function(x) #6.1
{
  for(row in 1:nrow(x)){
    for(col in 1:ncol(x[,1:4])){
      if(x[row,col]<=45){ #6.2.2
        print("Student at risk")
      }
      else if(x[row,col]>=45 && x[row,col]<61){ #6.2.3
        print("Student may be approved")
      }
      else{
        print("student approved") #6.2.3
      }
    }
  }
}

students_projection(result_students) #6.3
```
